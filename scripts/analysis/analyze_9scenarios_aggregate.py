#!/usr/bin/env python3
"""
Aggregate analysis of emcee results across all 9 scenarios.
"""
...
SCENARIO_LABELS = {
    ...
    'simple_average_ct': '9. Simple Avg + CT (no MCMC)',
}

def load_results(base_path='/eos/project-e/ep-nu/evilla/sn-pointing', min_es_files=9):
    """Load all emcee results for the 9 scenarios.
    ...
    for cat_dir in sorted(base.glob('cat*')):
        ...
        for scenario in SCENARIOS:
            ...
            # If not found in EOS structure, try dataset location
            if not emcee_file.exists():
                alt_pipeline_dir = Path(f"/eos/user/e/evilla/supernova/dataset_9_7_24_with_positions/{cat_name}/pipeline")
                emcee_file = alt_pipeline_dir / f"{cat_name}_scenario_{scenario}_emcee.npz"
            
            if emcee_file.exists():
                try:
                    data = np.load(emcee_file)
                    # Try both key formats (MCMC has _emcee_ prefix, non-MCMC doesn't)
                    ...
                    # Fallback to non-MCMC keys
                    if key_error not in data:
                        key_error = f"{scenario}_angular_error"
                    if key_cos_theta not in data:
                        key_cos_theta = f"{scenario}_cos_theta"
                    if key_n_clusters not in data:
                        key_n_clusters = f"{scenario}_n_clusters_used"
                    ...
                    if key_error in data and key_cos_theta in data:
                        ...
                        # Store true neutrino direction (try both formats)
                        key_true_px = f"{scenario}_emcee_true_nu_px"
                        key_true_py = f"{scenario}_emcee_true_nu_py"
                        key_true_pz = f"{scenario}_emcee_true_nu_pz"
                        if key_true_px not in data:
                            key_true_px = f"{scenario}_true_nu_px"
                            key_true_py = f"{scenario}_true_nu_py"
                            key_true_pz = f"{scenario}_true_nu_pz"
                        ...
                        if key_n_clusters in data and n_total_clusters > 0:
                            ...
                            # Track ES and CC cluster counts (try both key formats)
                            key_n_es = f"{scenario}_emcee_n_es_clusters"
                            key_n_cc = f"{scenario}_emcee_n_cc_clusters"
                            if key_n_es not in data:
                                key_n_es = f"{scenario}_n_es_clusters"
                                key_n_cc = f"{scenario}_n_cc_clusters"
                            if key_n_es in data and key_n_cc in data:
                                cluster_tracking[scenario]['n_es'].append(int(data[key_n_es]))
                                cluster_tracking[scenario]['n_cc'].append(int(data[key_n_cc]))
    ...
    for scenario in SCENARIOS:
        ...
        if len(t['n_used']) > 0:
            ...
            track_stats[scenario] = {
                ...
                'used_mean': mean_used,
                'es_mean': np.mean(t['n_es']) if 'n_es' in t and len(t['n_es']) > 0 else 0,
                'cc_mean': np.mean(t['n_cc']) if 'n_cc' in t and len(t['n_cc']) > 0 else 0,
...
def plot_aggregate_analysis(results, cat_names, cos_theta_results, cluster_tracking, cat_file_stats, true_directions):
    ...
    output_pdf = 'emcee_9scenarios_aggregate_analysis.pdf'
    ...
    for i, scenario in enumerate(SCENARIOS):
        if stats[scenario] is not None:
            ...
            ts = track_stats.get(scenario)
            if ts:
                n_es = ts['es_mean']
                n_cc = ts['cc_mean']
                title = f'{SCENARIO_LABELS[scenario]}\n(N_cats={n_cats}, N_ES={n_es:.0f}, N_CC={n_cc:.0f})'
            else:
                title = f'{SCENARIO_LABELS[scenario]}\n(N_cats={n_cats})'
            ax.set_title(title,
...
def main():
    print("Loading emcee results for 9 scenarios...")
