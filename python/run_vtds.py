...
def run(input_data, output_folder):
    vtds_parameters = input_data["vtds"]

    filename = output_folder+vtds_parameters["filename"]
    vtds_outfolder = output_folder+vtds_parameters["output_folder"]
    width = vtds_parameters["img_witdh"]
    height = vtds_parameters["img_height"]
    x_margin = vtds_parameters["x_margin"]
    y_margin = vtds_parameters["y_margin"]
    which_detector = vtds_parameters["which_detector"]
    min_tps_to_cluster = vtds_parameters["min_tps_to_cluster"]
    save_img_dataset = vtds_parameters["save_img_dataset"]
    save_process_label = vtds_parameters["save_process_label"]
    save_true_dir_label = vtds_parameters["save_true_dir_label"]
    ...
    if not os.path.exists(vtds_outfolder + 'dataset'):
        os.makedirs(vtds_outfolder + 'dataset')
    ...
    if save_img_dataset:
        ...
        np.save(vtds_outfolder + 'dataset/dataset_img.npy', dataset_img)
        save_samples_from_ds(dataset_img, vtds_outfolder + 'samples/', n_samples=10)
    # Create the labels
    if save_process_label:
        ...
        np.save(vtds_outfolder + 'dataset/dataset_label_process.npy', dataset_label_process)
    if save_true_dir_label:
        ...
        np.save(vtds_outfolder + 'dataset/dataset_label_true_dir.npy', dataset_label_true_dir)
