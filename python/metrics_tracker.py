...
class MetricsTracker:
    ...
    def add_mt_identification_metrics(self, mt_results):
        """Add metrics from MT identification step."""
        metrics = mt_results['metrics']
        
        self.metrics['steps']['mt_identification'] = {
            'input_clusters': len(mt_results['y_true']),
            'predicted_mt': mt_results['n_predicted_mt'],
            'confusion_matrix': {
    ...
    def get_summary(self):
        ...
        # MT identification
        if 'mt_identification' in self.metrics['steps']:
            mt = self.metrics['steps']['mt_identification']
            lines.append("Main Track Identification:")
            lines.append(f"  Input Clusters: {mt.get('input_clusters', 'N/A')}")
            lines.append(f"  Predicted as MT: {mt.get('predicted_mt', 'N/A')}")
            if 'performance' in mt:
                perf = mt['performance']
