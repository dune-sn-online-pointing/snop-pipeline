#!/usr/bin/env python3
"""
Run the ED model on clusters selected by the main-track (MT) identifier.

This script expects three inputs:
 - `volumes_npz`: .npz with `volumes` array shaped (N, ...), one volume per cluster.
 - `mt_results_npz`: .npz with a boolean mask `is_main_track` (N,) and a `tentative_dirs` array
...
def main():
    ...
    p.add_argument('mt_results_npz', help='.npz file containing `is_main_track` boolean mask and optionally `tentative_dirs`')
    p.add_argument('--out', default='results/ed_inference_on_mt.npz')
    ...
    tentative_dirs = mt_data.get('tentative_dirs')

    if volumes.shape[0] != is_main.shape[0]:
        raise ValueError('volumes and is_main_track must have the same length')

    selected_idx = np.where(is_main)[0]
    print(f'Found {len(selected_idx)} main-track clusters')
    ...
    print(f'Saved ED inference for MT clusters to {out_path}')
